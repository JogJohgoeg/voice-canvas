...
GPU plugins and Connectome are inside the workbench. Choose an included shader, edit its sliders, save it to the local gallery, or export/import a single JSON file. Turn off Zero-AI to request a GPT-6 shader; generation is asynchronous (measured 25.2 s for a 1,191-character shader), then the browser compiles and checks it. Unknown JS init is rejected; this version accepts fragment-only plugins. Enable Connectome for the real MaleCNS graph; its visible label distinguishes estimated layout from anatomy. PNG/WebM and projector include these layers while they remain active. A plugin that exceeds budget is stopped, including during recording; tier 1 continues.
