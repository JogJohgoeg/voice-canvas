...
const metrics={parser_ms:0,tier1_paint_ms:0,model_paint_ms:null};const view=renderer($('canvas'),()=>preview??state,()=>({style,pianoParts:Number($('pianoParts').value),pianoRandom:$('pianoRandom').checked,monet:$('monetBlend').checked,hasModel:hasModel||blender.visible,evolution,sound,volume:Number($('volume').value)}),micAudio);
...
$('soundMode').onchange=()=>{sound.setMode($('soundMode').value);$('pianoParts').hidden=$('pianoRandomControl').hidden=$('soundMode').value!=='cinematic';};
