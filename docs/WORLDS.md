...
The page opens with the cached Opening world as its default background. A first scene click starts the saved original piano phrase; it is not a recording or a transcription of Mishima. Playback loops locally without model calls. World remains selectable through the same one-prompt performance page. Try **“Glass Mishima Opening World”**, or **“Glass Mishima Opening World 零AI”** to play through MIDI input without a director request. Add “麦克风” for acoustic input. The camera automatically completes full panoramic turns and follows a slow curved path near its starting position. Dynamics adjust tour speed; cadences ease into a different viewing direction. Drag and scroll remain optional offsets. The projector and recording use the same world + particle composition.
