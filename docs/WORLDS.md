...
The page opens with the cached Opening world as its default background. A first scene click starts original piano variations; the local arranger creates a different 16-beat phrase each round, with a fresh music seed per visit. These are not recordings or a transcription of Mishima. Playback runs locally without model calls. World remains selectable through the same one-prompt performance page. Try **“Glass Mishima Opening World”**, or **“Glass Mishima Opening World 零AI”** to play through MIDI input without a director request. Add “麦克风” for acoustic input. Use the top-right **切换背景** button or **B** to cycle cached worlds; the selection becomes this browser’s default after reload. Typing B in the prompt does not switch backgrounds. The camera automatically completes full panoramic turns and follows a slow curved path near its starting position. Dynamics adjust tour speed; cadences ease into a different viewing direction. Drag and scroll remain optional offsets. The projector and recording use the same world + particle composition.
